...
NOTEBOOK_PATHS = {
    "variable": NOTEBOOK_DIR / "gpt2-variable-position.ipynb",
    "fixed": NOTEBOOK_DIR / "gpt2-fixed-position.ipynb",
}
...
def test_notebooks_use_gpt2_eos_readout_and_dedicated_padding():
    for variant in NOTEBOOK_PATHS:
        source = notebook_source(load_notebook(variant))

        assert 'MODEL_NAME = "openai-community/gpt2"' in source
        assert "CONTEXT_LENGTH = 1024" in source
        assert "MAX_TEXT_LENGTH = CONTEXT_LENGTH - 1" in source
        assert 'tokenizer.add_special_tokens({"pad_token": PAD_TOKEN})' in source
        assert "model.resize_token_embeddings(len(tokenizer))" in source
...
def test_notebooks_calibrate_evaluate_and_export_distinct_artifacts():
    expected = {
        "variable": (
            "gpt2-variable-position-confmat.svg",
            'Path("artifacts") / "gpt2-variable-position-ai-detector"',
        ),
        "fixed": (
            "gpt2-fixed-position-confmat.svg",
            'Path("artifacts") / "gpt2-fixed-position-ai-detector"',
        ),
    }
